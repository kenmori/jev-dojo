...
function toRequest(input: string, init?: RequestInit): FixtureRequest {
...
export function createReplayFetch(step: string, onReplay?: (meta: FixtureMeta) => void): Fetch {
...
export function createRecordingFetch(step: string, realFetch: Fetch = fetch): Fetch {
  return async (input, init) => {
...
export function fetchFromResponses(...responses: FixtureResponse[]): Fetch & { calls: number } {
  let i = 0;
  const f = async () => {
