...
全サンプルのリクエストを1回ずつ live で送ったときの Jev の費用の目安: 約 $0.0024（入力 58,149 トークン、新規クレジット $5 の 0.049%）。※ 見本データ（合成）のトークン数からの推定。応用B の Claude の費用は別にかかります
<!-- facts:end -->
...
入門（級）・中級（段）・上級（高段）・発展（応用A〜D）の全章がそろっています。計画の全体は [plan.md](plan.md) を参照してください。
