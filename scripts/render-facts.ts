...
export function renderReadmeBlock(f: Facts, docMapCheckedAt: string | null): string {
  ...
  return [
    ...
    "```",
    "",
    `全サンプルを live で1周したときの費用の目安: 約 ${formatUSD(est.costUSD)}（入力 ${est.inputTokens.toLocaleString("en-US")} トークン、新規クレジット ${f.signup.startingCredit} の ${((est.costUSD / Number(f.signup.startingCredit.replace(/[^\d.]/g, ""))) * 100).toPrecision(2)}%）${est.synthetic ? "。※ 見本データ（合成）のトークン数からの推定" : ""}`,
    "<!-- facts:end -->",
  ].join("\n");
}
