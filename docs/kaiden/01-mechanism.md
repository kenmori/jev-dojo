...
| **［計画書］** | この教材の計画（plan.md）の調査で公式ページから引用されたもの。作成時の環境では原文と照合できていない |
...
**［確認］** Choice / Score の confidence は「分布の集中度を要約したもの」で、ワークフロー全体の正しさや、実行してよいという許可ではない（SKILL.md）。
...
**いま入っている応答は見本データ（合成）です。** 見本は教材側の式で作っているので、「観察」の数字は見本の作り方を映しているだけです。`npm run record` で実APIの応答に置き換えてから実行すると、はじめて Jev についての証拠になります。
...
- **［計画書］** 英語が主要な学習言語で、他の言語は同等とは限らない。英語以外で使うなら自分のコンテンツでテストすること（公式ドキュメントの Models ページ）
