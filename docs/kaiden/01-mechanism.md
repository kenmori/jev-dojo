...
**［確認］** confidence は、返ってきた確率の分布から計算される（公式の Confidence のページ）。公式の 3 択の説明では「(3 × 1 位の確率 − 1) ÷ 2」で近似している。選択肢が K 個なら次の形です。

```
confidence ≒ (K × 1位の確率 − 1) ÷ (K − 1)
```

記録した Choice の答えも、この形とほぼ一致します（`npm run okugi -- --live-only`）。上のエントロピーの式とは別の物差しです。

**［不明］** confidence の正確な計算式。公式も「近似」としか書いていません。Score の confidence は、この式と合わない答えもあります。
...
- 確率は小数 2 桁に丸めて返ります。記録した 616 組の答えでは合計はすべて 1.00 でしたが、丸めた値どうしを足すので、ちょうど 1 になる保証はありません。テストで合計を比べるときは許容幅を持たせます
...
- **［確認］** Jev は RLCD（較正された判断のための強化学習）で、文章ではなく判断と較正された確率を返すように訓練されている。顧客のデータでは追加の学習をせず、すべてのアカウントで同じモデルを使う（公式ドキュメントの Models と AI primer のページ）
- **［一般論］** 「System 1 / System 2」は心理学者ダニエル・カーネマンの区別
- **［不明］** モデルの構造、パラメータ数、訓練データ、訓練のくわしい手順、confidence の正確な計算式、criteria の文章がどう使われているか
